...
namespace :coteyr_pack do
  ...
  task :setup do
    ...
    FileUtils.cp_r( Dir[File.join(plugin_dir, 'public')], File.join(::Rails.root.to_s), :verbose => true)
    FileUtils.cp_r( Dir[File.join(plugin_dir, 'config')], File.join(::Rails.root.to_s), :verbose => true)
    FileUtils.cp( File.join(plugin_dir, '../.gitignore'), File.join(::Rails.root.to_s), :verbose => true)
    FileUtils.cp( File.join(plugin_dir, '../Capfile'), File.join(::Rails.root.to_s), :verbose => true)
    FileUtils.cp_r( Dir[File.join(plugin_dir, 'script')], File.join(::Rails.root.to_s), :verbose => true)
    puts "Finished."
  end
  desc "Copy Application.html.erb and other first time files"
  task :first_time do
    plugin_dir = File.join(File.dirname(__FILE__), '../')
    FileUtils.cp_r( Dir[File.join(plugin_dir, 'app')], File.join(::Rails.root.to_s), :verbose => true)
    FileUtils.cp_r( Dir[File.join(plugin_dir, 'app')], File.join(::Rails.root.to_s), :verbose => true)
    FileUtils.cp( File.join(plugin_dir, '../Gemfile'), File.join(::Rails.root.to_s), :verbose => true)
  end
  namespace :linux do
    ...
    task :console do
      sh ("konsole -workdir #{::Rails.root.to_s} -e rails console")
    end
    ...
    task :server do
      sh("konsole -workdir #{::Rails.root.to_s} -e rails server")
    end
  end
  namespace :log do
    desc "Statitcs"
